...
class ShapNN(object):
    ...
    def _build_train_op(self):
        ...
        grads = tf.gradients(ys=self.loss, xs=trainable_variables)
    ...
    def _final_layer(self, x, num_classes, mode):
        ...
        if mode=='regression':
            ...
            self.predictions = tf.reduce_sum(input_tensor=self.logits, axis=-1)
            regression_loss = tf.nn.l2_loss(self.predictions - self.labels) ##FIXIT
            self.prediction_loss = tf.reduce_mean(input_tensor=regression_loss, name='l2')
            residuals = self.predictions - self.labels
            var_predicted = tf.reduce_mean(input_tensor=residuals**2) - tf.reduce_mean(input_tensor=residuals)**2
            var_labels = tf.reduce_mean(input_tensor=self.labels**2) - tf.reduce_mean(input_tensor=self.labels)**2
            ...
            self.prediction_loss = tf.reduce_mean(input_tensor=xent_loss, name='xent')
            self.predictions = tf.argmax(input=self.probs, axis=-1, output_type=tf.int32)
            correct_predictions = tf.equal(self.predictions, self.labels)
            self.prediction_score = tf.reduce_mean(input_tensor=tf.cast(correct_predictions, tf.float32))
    ...
    def _dense(self, x, out_dim, dropout=tf.constant(0.), initializer=None, activation=None):
        ...
        x = tf.nn.dropout(x, rate=1 - (1. - dropout))
        if activation:
            x = activation(x)
...
class CShapNN(ShapNN):
    ...
    def _conv(self, x, filter_size, out_filters, strides, activation=None):
        ...
        x = tf.nn.conv2d(input=x, filters=kernel, strides=strides, padding='SAME')
        if activation:
            x = activation(x)
        return x
